...
public class Initialise {
    ...
    public void sqlConfiguration() {
        ...
        if (!sqlConfig.exists()) {
            try {
                ...
                c.set("Database.Type.SQLite.UseSQLite", true);
                c.set("Database.Type.MySQL.UseMySQL", false);
                c.set("Database.Type.MySQL.Host", "localhost");
                c.set("Database.Type.MySQL.Username", "admin");
                c.set("Database.Type.MySQL.Databasename", "root");
                c.set("Database.Type.MySQL.Password", "1234");
                c.save(sqlConfig);
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        try {
            ...
            Logger.getLogger(Initialise.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void SQLite_Startup() {
        ...
        if (!core.checkTable("statistics")) {
            ...
            /**
             * TODO: Add Kill Death Points Maybe, buy features...
             */
    ...
    public static Float getPlayerScore(int wins, int draws, int losses, int shotshit, int kills, int deaths) {
        /*
         * Win = +500 Draw = +200 Loss = +50 ShotHit = +5 Kill = +30 Death = +5
         */
        Float score = (float) 0;
        score += (wins * 500);
        score += (draws * 200);
